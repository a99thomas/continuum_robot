...
def animate():
    ...
    def frame_update(frame):
        nonlocal initial_guess, prev_time, prev_tendon_lengths, config
        ...
        axes, buttons, _ = controller.get_transmitter_values()

        # Update target pose based on input
        dt = (time.time() - prev_time) / 40
        prev_time = time.time()
        target_pose[:3, 3] += [axes["L1"] * dt, axes["L2"] * dt, (buttons["Y"] - buttons["A"]) * dt]

        pitch_change = axes["R1"] * dt * 300
        yaw_change   = axes["R2"] * dt * 300
        ...
        radius = [0.0254] * len(ell_init)
        tendon_lengths = config_to_lengths(config=config, r=radius)
        ...
        task_coords = config_to_task(config, pts_per_seg=num_segments)
        plot_elements = plot_tf(ax, task_coords, seg_end, tipframe=True, segframe=False,
                                baseframe=True, projections=True, baseplate=True)
        update_plot(plot_elements, task_coords)
        # print(config[1,:])
        draw_tdcr(ax, task_coords, seg_end, r_disk=1.5e-2, r_height=1.5e-3,
